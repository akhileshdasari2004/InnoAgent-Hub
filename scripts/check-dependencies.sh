...
check_dependencies() {
    ...
    if [ "$missing_deps" -eq 1 ]; then
        ...
        echo -e "  - ${C_YELLOW}OpenAI API Key${C_NC}: Required for agents (set in config/.env)"
        echo -e "  - ${C_YELLOW}GitHub Personal Access Token${C_NC}: Required for GitHub & Community agents (set in config/.env)"
        echo -e "  - ${C_YELLOW}Firecrawl API Key${C_NC}: Required for Firecrawl agent (set in config/.env)"
        exit 1
    else
        echo -e "${C_GREEN}✅ All dependencies meet version requirements!${C_NC}"
        echo -e "\n${C_BLUE}Additional requirements to prepare:${C_NC}"
        echo -e "  - ${C_YELLOW}OpenAI API Key${C_NC}: Required for agents (set in config/.env)"
        echo -e "  - ${C_YELLOW}Firecrawl API Key${C_NC}: Required for web scraping agent (set in config/.env)"
        echo -e "  - ${C_YELLOW}GitHub Personal Access Token${C_NC}: Required for GitHub & Community agents (set in config/.env)"
    fi
}
